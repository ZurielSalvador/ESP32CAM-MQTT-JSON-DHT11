 ...
 * Actualizado por: Victor Zuriel Dominguez Salvador. 
 * Fecha: 16 de agosto de 2021

 * 
 * Este programa realiza la lectura del DHT11 con ayuda de la biblioteca AdaFruit
 * y envía la temperatura y humedad relativa por MQTT en un JSON.
 * 
 * Este programa está basado en el programa MQTT basic y JSON basic del repositorio
 * de CodigoIoT
 * 
...
#define DHTPIN 13 //Pin para conectar el DHT11, Pin GPIO13 del ESP32-CAM
#define DHTTYPE DHT11 //Configurar el modelo del sensor.
...
DHT dht(DHTPIN, DHTTYPE); // Este objeto inicia el sensor
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    //Con esto se obtiene temperatura y humedad
    float t = dht.readTemperature();
    float h = dht.readHumidity();

    //Secuencia que se asegura que la conexión con el sensor exista.
    if (isnan(h) || isnan(t)) {
    ...
    String json = "{\"id\":\"Zuriel\",\"temp\":"+String(t)+",\"hum\":"+String(h)+"}";
